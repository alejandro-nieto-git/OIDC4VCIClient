...
export class CredentialRequestClientBuilder {
  ...
  public static async fromURI({ credentialDefinition, uri, metadata }: { credentialDefinition: JsonLdIssuerCredentialDefinition, uri: string; metadata?: EndpointMetadata }): Promise<CredentialRequestClientBuilder> {
    const offer = await CredentialOfferClient.fromURI(uri);
    return CredentialRequestClientBuilder.fromCredentialOfferRequest({ credentialDefinition, request: offer, ...offer, metadata, version: offer.version });
  }
